# Сайты eyepoint.physlab.ru и usbadc10.physlab.ru

В этом репозитории находятся два сайта: eyepoint.physlab.ru и usbadc10.physlab.ru, каждый их них лежит в отдельной директории внутри папки sites. В прод сайты запускаются с помощью глобального docker-compose-файла на сервере vili.

Каждый сайт использует site_engine - набор общего кода, который используется каждым сайтом.

Сайты с каталогом товаров и файлами для скачивания софта под каждый товар. Один сайт - одна линейка товаров.
...
Так же в docker-compose на сервере должен быть build с указанным Dockerfile. Путь к Dockerfile зависит от имени сайта.
